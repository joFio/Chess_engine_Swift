...
struct BitboardSetup {
    ...
    static let WhitePawns:[UInt64] = [256,512,17179869184,2048,4096,8192,16384,32768]
    ...
    static let BlackPawns:[UInt64] = [281474976710656,562949953421312,1125899906842624,134217728,4503599627370496,9007199254740992,18014398509481984,36028797018963968]
}
...
struct Board{
    ...
    // Return bitboards of Pawn(true), then check against this for move
    // Create prive class Info
   ...
   mutating func nextTurn(){
    ...
    static func newBoardForMove(fromCase:UInt64, toCase:UInt64, team:Bool,board:Board, info:Info)throws ->Board{
        ...
        let adversaryBitboard:UInt64 = info.getAdversaryBitboard(team: team)
        var newBoard = board
        ...
        if adversaryBitboard & toCaseBitboard != 0 {
...
struct Piece {
    ...
    static func getPieceAtPositionInBoard(board:Board, position:UInt64)->Piece?{
...
let piece = game.getPieceAtPosition(position: 34)!.1
